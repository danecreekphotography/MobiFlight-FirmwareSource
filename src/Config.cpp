    ...
    void readConfigFromFlash()
    {
        ...
        do // go through the Flash until it is NULL terminated
        {
            ...
            case kTypeOutput:
                params[0] = readUintFromFlash(&addrFlash); // Pin number
                Output::Add(params[0]);
                copy_success = readEndCommandFromEEPROM(&addrFlash, ':'); // check EEPROM until end of name
                break;
            ...
            case kTypeAnalogInput:
                ...
                copy_success = readNameFromFlash(&addrFlash, nameBuffer, &addrbuffer); // copy the NULL terminated name to to nameBuffer and set to next free memory location
                                                                                        //    copy_success = readEndCommandFromEEPROM(&addrFlash, ':');       // once the nameBuffer is not required anymore uncomment this line and delete the line before
                break;
    #endif
            ...
            case kTypeCustomDevice: {
                ...
                copy_success     = readEndCommandFromEEPROM(&addrFlash, '.');
                if (!copy_success)
                    break;
                ...
                if (copy_success) {
                    ...
                    copy_success = readEndCommandFromEEPROM(&addrFlash, ':'); // check EEPROM until end of command
                }
